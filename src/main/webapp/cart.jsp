...
<div class="header">
    <div class="header_main">
        ...
        <div class="right">
            ...
            <span class="user_name"><a href="">${sessionScope.userInfo.name}</a></span>
        </div>
    </div>
</div>
<!--主要显示栏-->
    ...
    <c:if test="${cartList.size()!=0}">
            ...
            <div class="order_item">
                <div class="select">
                    <c:if test="${cart.flag==1}">
                        <img src="pic/other/checked.png" onclick="window.location.href='select?id=${cart.id}'"
                             width="18" height="18">
                    </c:if>
                    <c:if test="${cart.flag==2}">
                        <img src="pic/other/kong.png" onclick="window.location.href='select?id=${cart.id}'"
                             onmouseenter="this.src='pic/other/hover.png';" onmouseleave="this.src='pic/other/kong.png'"
                             width="18" height="18">
                    </c:if>
                </div>
                <div class="pic"><img src="http://123.57.236.58/${cart.good_pic}" width="80" height="80"/></div>
                <div class="detail">${cart.good_name}</div>
                <div class="price">${cart.good_price}元</div>
                <div class="num">
                    <a id="divNum" href="divNum?user_id=${cart.user_id}&good_id=${cart.good_id}">
                        <input id="sub" type="button" value="-">
                    </a>
                    <input id="buy_num" type="text" disabled="disabled" value="${cart.num}" style="cursor: text" name="buy_num" width="90">
                    <a href="addNum?user_id=${cart.user_id}&good_id=${cart.good_id}">
        ...
        <div class="pay">
            <div class="pay_info">
                ...
                <span class="static">共<span class="baby_count">${sum}</span>件商品，已选择<span
